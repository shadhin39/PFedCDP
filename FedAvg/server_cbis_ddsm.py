# server_cbis_ddsm.py
# Federated Learning Server for CBIS-DDSM Dataset
...
# Custom strategy for CBIS-DDSM
class CBISFedAvg(FedAvg):
    ...
    def aggregate_evaluate(
        ...
        if aggregated_metrics:
            ...
            if "auc" in aggregated_metrics:
                self.round_aucs.append(aggregated_metrics["auc"])
            if "specificity" in aggregated_metrics:
                self.round_specificities.append(aggregated_metrics["specificity"])
            if "sensitivity" in aggregated_metrics:
                self.round_sensitivities.append(aggregated_metrics["sensitivity"])
            
        # Print round summary
        print(f"\n=== CBIS-DDSM Round {server_round} Results ===")
        ...
        print(f"Aggregated AUC: {aggregated_metrics.get('auc', 0):.4f}")
        print(f"Aggregated specificity: {aggregated_metrics.get('specificity', 0):.4f}")
        print(f"Aggregated sensitivity: {aggregated_metrics.get('sensitivity', 0):.4f}")
        print(f"Number of clients: {len(results)}")
        
        # Save results to file
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        with open(f"cbis_ddsm_round_{server_round}_{timestamp}.txt", "w") as f:
            ...
            f.write(f"AUC: {aggregated_metrics.get('auc', 0)}\n")
            f.write(f"Specificity: {aggregated_metrics.get('specificity', 0)}\n")
            f.write(f"Sensitivity: {aggregated_metrics.get('sensitivity', 0)}\n")
...
if __name__ == "__main__":
    print("Starting CBIS-DDSM Federated Learning Server...")
    ...
    print("\nCBIS-DDSM Federated Learning completed!")
