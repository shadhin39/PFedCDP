...
# Add MedMNIST import after existing imports
import medmnist
...
def partition_data_medmnist(client_id):
    """Load and partition ChestMNIST data for federated learning"""
    ...
    # Get data and labels
    x_train_full = train_dataset.imgs
    y_train_full = train_dataset.labels.flatten()
    x_test_full = test_dataset.imgs
    y_test_full = test_dataset.labels.flatten()
    
    # Simple partitioning for federated learning
    num_clients = 10
    train_size = len(x_train_full)
    partition_size = train_size // num_clients
    
    start_idx = client_id * partition_size
    end_idx = start_idx + partition_size if client_id < num_clients - 1 else train_size
    
    # Client's training data
    x_train = x_train_full[start_idx:end_idx]
    y_train = y_train_full[start_idx:end_idx]
    
    # Use a portion of test data for each client
    test_size = len(x_test_full)
    test_partition_size = test_size // num_clients
    test_start = client_id * test_partition_size
    test_end = test_start + test_partition_size if client_id < num_clients - 1 else test_size
...
x_train, y_train, x_test, y_test = partition_data_medmnist(client_id)
...
class KnowledgeDistillationLoss:
    ...
    def compute_distillation_loss(self, student_logits, teacher_logits, true_labels):
        ...
        if len(true_labels.shape) == 1:
            true_labels_onehot = tf.one_hot(true_labels, depth=14)  # Changed from 10 to 14
        else:
...
# Partitioned Model C with ultra-light architecture
# Add Excel export functionality to PartitionedModelC class
# Add MedMNIST import and update PartitionedModelC
class PartitionedModelC:
    def __init__(self, input_shape=(28, 28, 1), num_classes=14, client_id=None):  # ChestMNIST has 14 classes
        self.client_id = client_id
        # Feature Extractor (W_L,k) - shared with server (ultra-light architecture)
        self.feature_extractor = keras.Sequential([
            keras.Input(shape=input_shape),
            layers.Conv2D(8, kernel_size=(5, 5), activation="relu"),  # Even fewer filters
            layers.MaxPooling2D(pool_size=(2, 2)),
            layers.Conv2D(16, kernel_size=(3, 3), activation="relu"),  # Reduced from 32
            layers.MaxPooling2D(pool_size=(2, 2)),
            layers.Flatten(),
            layers.Dense(32, activation="relu")  # Much smaller dense layer
        ])
    ...
    def train_with_distillation(self, x_train, y_train, epochs=5, batch_size=64, current_round=1):
        ...
        # Convert labels to one-hot (UPDATE: ChestMNIST has 14 classes)
        y_train_onehot = tf.one_hot(y_train, depth=14)
            ...
            for epoch in range(epochs):
                ...
                for batch_idx, (batch_x, batch_y) in enumerate(dataset):
                    ...
                    if (batch_idx + 1) % 10 == 0 or (batch_idx + 1) == total_batches:
                        ...
                        print(f"\r[CLIENT C-{self.client_id}] {batch_idx + 1}/{total_batches} [{progress}{remaining}] - {batch_time*1000:.0f}ms/step", end="", flush=True)
                ...
                # Final epoch summary
                print(f"\n[CLIENT C-{self.client_id}] {epoch_time:.0f}s {epoch_time*1000/num_batches:.0f}ms/step - total_loss: {avg_loss:.4f} - ce_loss: {avg_ce_loss:.4f} - kd_loss: {avg_kd_loss:.4f} - accuracy: {avg_accuracy:.4f}")
                ...
                # Log to file with detailed format
                with open(f"client_C_epochs.txt", "a") as f:
                    f.write(f"[CLIENT C-{self.client_id}] Epoch {epoch + 1}/{epochs} - {epoch_time:.0f}s {epoch_time*1000/num_batches:.0f}ms/step - total_loss: {avg_loss:.4f} - ce_loss: {avg_ce_loss:.4f} - kd_loss: {avg_kd_loss:.4f} - accuracy: {avg_accuracy:.4f}\n")
                
        except tf.errors.OutOfRangeError:
            print(f"\n[CLIENT C-{self.client_id}] Dataset iteration completed successfully")
        except Exception as e:
            print(f"\n[CLIENT C-{self.client_id}] Training error: {e}")

# Define Flower client
class FlowerClient(fl.client.NumPyClient):
    def __init__(self, client_id):
        self.client_id = client_id
        self.partitioned_model = PartitionedModelC(client_id=client_id)
        ...
        with open("client_C_epochs.txt", "w") as f:
    ...
    def fit(self, parameters, config):
        print(f"\n{'='*70}")
        print(f"[CLIENT C-{self.client_id}] STARTING ROUND {self.current_round}")
        print(f"{'='*70}")
        ...
        with open("client_C_epochs.txt", "a") as f:
            f.write(f"\n[CLIENT C-{self.client_id}] ROUND {self.current_round} COMPLETED\n" + "="*60 + "\n\n")
    ...
    def evaluate(self, parameters, config):
        ...
        try:
            ...
            with open("client_C_epochs.txt", "a") as f:
                f.write(f"[CLIENT C-{self.client_id}] Evaluation Round {self.current_round - 1}: Loss={loss:.4f}, Accuracy={accuracy:.4f}\n")
            
            print(f"[CLIENT C-{self.client_id}] Evaluation Results - Loss: {loss:.4f}, Accuracy: {accuracy:.4f}")
...
# Connect to server on port 8082
...
def export_metrics_to_excel(self):
    """Export client metrics to Excel"""
    try:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        df = pd.DataFrame(self.epoch_metrics)
        df.to_excel(f'client_C_{self.client_id}_metrics_{timestamp}.xlsx', index=False)
        print(f"[Excel Export] Client C-{self.client_id} metrics exported")
    except Exception as e:
        print(f"Error exporting Client C metrics: {e}")