...
def partition_data_medmnist(client_id):
    ...
    # Simple partitioning for federated learning (you can implement Dirichlet if needed)
    num_clients = 10
...
# Update PartitionedModelB class
...
class FlowerClient(fl.client.NumPyClient):
    def __init__(self, client_id):
        self.client_id = client_id
        self.partitioned_model = PartitionedModelB(client_id=client_id)
        ...
        with open("client_B_epochs.txt", "w") as f:
    ...
    def fit(self, parameters, config):
        print(f"\n{'='*70}")
        print(f"[CLIENT B-{self.client_id}] STARTING ROUND {self.current_round}")
        print(f"{'='*70}")
        ...
        with open("client_B_epochs.txt", "a") as f:
            f.write(f"\n[CLIENT B-{self.client_id}] ROUND {self.current_round} COMPLETED\n" + "="*60 + "\n\n")
    ...
    def evaluate(self, parameters, config):
        ...
        try:
            ...
            with open("client_B_epochs.txt", "a") as f:
                f.write(f"[CLIENT B-{self.client_id}] Evaluation Round {self.current_round - 1}: Loss={loss:.4f}, Accuracy={accuracy:.4f}\n")
            
            print(f"[CLIENT B-{self.client_id}] Evaluation Results - Loss: {loss:.4f}, Accuracy: {accuracy:.4f}")
...
fl.client.start_numpy_client(server_address="127.0.0.1:8081", client=FlowerClient(client_id))


# Add Excel export functionality to PartitionedModelB class
def export_metrics_to_excel(self):
    """Export client metrics to Excel"""
    try:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        df = pd.DataFrame(self.epoch_metrics)
        df.to_excel(f'client_B_{self.client_id}_metrics_{timestamp}.xlsx', index=False)
        print(f"[Excel Export] Client B-{self.client_id} metrics exported")
    except Exception as e:
        print(f"Error exporting Client B metrics: {e}")