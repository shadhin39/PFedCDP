...
class PartitionedModelB:
    ...
    def train_with_distillation(self, x_train, y_train, epochs=5, batch_size=64, current_round=1):
        ...
        if self.teacher_model is None:
            ...
            print(f"[CLIENT B-{self.client_id}] Round {current_round}: Training without distillation")
            ...
            class EpochTracker(keras.callbacks.Callback):
                ...
                def on_epoch_end(self, epoch, logs=None):
                    ...
                    with open(f"client_B_epochs.txt", "a") as f:
                        f.write(f"[CLIENT B-{self.model_instance.client_id}] Round {self.round_num}, Epoch {epoch + 1}: Loss={loss:.4f}, Accuracy={accuracy:.4f}\n")
                    
                    print(f"[CLIENT B-{self.model_instance.client_id}] Round {self.round_num}, Epoch {epoch + 1}: Loss={loss:.4f}, Accuracy={accuracy:.4f}")
            
            epoch_tracker = EpochTracker(self, current_round)
        ...
        print(f"[CLIENT B-{self.client_id}] Round {current_round}: Training with knowledge distillation")
        ...
        try:
            ...
            for epoch in range(epochs):
                print(f"[CLIENT B-{self.client_id}] Round {current_round}, Epoch {epoch + 1}/{epochs}")
                epoch_loss = 0
                epoch_ce_loss = 0
                epoch_kd_loss = 0
                epoch_accuracy = 0
                ...
                # Log to file
                with open(f"client_B_epochs.txt", "a") as f:
                    f.write(f"[CLIENT B-{self.client_id}] Round {current_round}, Epoch {epoch + 1}: Total Loss={avg_loss:.4f}, CE Loss={avg_ce_loss:.4f}, KD Loss={avg_kd_loss:.4f}, Accuracy={avg_accuracy:.4f}\n")
                
                print(f"[CLIENT B-{self.client_id}] Total Loss: {avg_loss:.4f}, CE Loss: {avg_ce_loss:.4f}, KD Loss: {avg_kd_loss:.4f}, Accuracy: {avg_accuracy:.4f}")
                
        except tf.errors.OutOfRangeError:
            print(f"[CLIENT B-{self.client_id}] Dataset iteration completed successfully")
        except Exception as e:
            print(f"[CLIENT B-{self.client_id}] Training error: {e}")
...
class FlowerClient(fl.client.NumPyClient):
    def __init__(self, client_id):
        ...
        with open("client_B_epochs.txt", "w") as f:
            f.write(f"CLIENT B (ID: {client_id}) EPOCH TRACKING\n")
            f.write("=" * 50 + "\n")
    ...
    def fit(self, parameters, config):
        ...
        with open("client_B_epochs.txt", "a") as f:
            f.write(f"[CLIENT B-{self.client_id}] ROUND {self.current_round} COMPLETED\n\n")
    ...
    def evaluate(self, parameters, config):
        ...
        try:
            loss, accuracy = self.partitioned_model.full_model.evaluate(x_test, y_test, verbose=0)
            
            # Log evaluation results
            with open("client_B_epochs.txt", "a") as f:
                f.write(f"[CLIENT B-{self.client_id}] Evaluation Round {self.current_round - 1}: Loss={loss:.4f}, Accuracy={accuracy:.4f}\n")
            
            print(f"[CLIENT B-{self.client_id}] Evaluation - Loss: {loss:.4f}, Accuracy: {accuracy:.4f}")
