...
# def partition_data_medmnist(client_id, image_size=28):
#     """Load ChestMNIST dataset for federated learning"""
#     try:
#         # Use ChestMNIST for client-side federated learning
...
#         x_train_full = train_dataset.imgs.astype("float32") / 255.0
#         y_train_full = train_dataset.labels.flatten()
#         x_test_full = test_dataset.imgs.astype("float32") / 255.0
...
#         # Simple partitioning for federated learning (divide data among clients)
#         num_clients = 10
...
#         test_start_idx = client_id * test_samples_per_client
#         test_end_idx = test_start_idx + test_samples_per_client
        
#         x_test = x_test_full[test_start_idx:test_end_idx]
#         y_test = y_test_full[test_start_idx:test_end_idx]
        
#         print(f"Client {client_id} - ChestMNIST partition: Train {x_train.shape}, Test {x_test.shape}")
#         print(f"Number of classes: {n_classes}, Task: {task}")
        
#         return x_train, y_train, x_test, y_test
        
#     except Exception as e:
#         print(f"Error loading ChestMNIST for client {client_id}: {e}")
#         raise

# Replace the function
def partition_cbis_ddsm_data(client_id, image_size=(224, 224)):
    """Load CBIS-DDSM dataset for federated learning (private client data)"""
    try:
        # Load full CBIS-DDSM dataset
        x_train_full, y_train_full, x_test_full, y_test_full = load_cbis_ddsm_dataset(image_size=image_size)
        ...
        print(f"Client {client_id} - CBIS-DDSM partition: Train {x_train.shape}, Test {x_test.shape}")
        print(f"Classes: Benign(0), Malignant(1)")
        
        return x_train, y_train, x_test, y_test
        
    except Exception as e:
        print(f"Error loading CBIS-DDSM for client {client_id}: {e}")
        raise

class PartitionedModel:
    def __init__(self, client_id, num_classes=2):  # Changed from 14 to 2 for benign/malignant
        self.client_id = client_id
        self.num_classes = num_classes
        
        # Create feature extractor for (224, 224, 1) input - CBIS-DDSM mammography images
        self.feature_extractor = keras.Sequential([
            layers.Input(shape=(224, 224, 1)),  # Changed from (28, 28, 1) to (224, 224, 1)
        ...
        # Client-specific classifier for CBIS-DDSM (2 classes: benign/malignant)
        self.classifier = keras.Sequential([
            layers.Dense(128, activation='relu'),
            layers.Dropout(0.3),
            layers.Dense(self.num_classes, activation='softmax')  # 2 classes for CBIS-DDSM
        ])
    ...
    def train_with_distillation(self, x_train, y_train, epochs=5, batch_size=64, current_round=1):
        ...
        # Convert labels to one-hot (UPDATE: Fashion-MNIST has 10 classes)
    ...
    def export_metrics_to_excel(self):
        ...
        try:
            ...
            df.to_excel(f'client_A_{self.client_id}_metrics_{timestamp}.xlsx', index=False)
            print(f"[Excel Export] Client A-{self.client_id} metrics exported")
        except Exception as e:
            print(f"Error exporting Client A metrics: {e}")
...
class FlowerClient(fl.client.NumPyClient):
    ...
    def evaluate(self, parameters, config):
        ...
        try:
            ...
            with open("client_A_epochs.txt", "a") as f:
                f.write(f"[CLIENT A-{self.client_id}] Evaluation Round {self.current_round - 1}:\n")
            ...
            print(f"[CLIENT A-{self.client_id}] Evaluation Results:")
...
# Move the client_id input and data loading to the beginning (after imports)
# Add user input for client ID
client_id = int(input("Enter client ID for Model A (e.g., 1, 2, 3...): "))
print(f"\n=== INITIALIZING CLIENT A (ID: {client_id}) ===")

# Load dataset using CBIS-DDSM 
x_train, y_train, x_test, y_test = load_cbis_ddsm_dataset()

# Knowledge Distillation Loss Function (move this before PartitionedModel class)
class KnowledgeDistillationLoss:
    ...
    def compute_distillation_loss(self, student_logits, teacher_logits, true_labels):
        ...
        if len(true_labels.shape) == 1:
            true_labels_onehot = tf.one_hot(true_labels, depth=2)  # Changed from 10 to 2 for CBIS-DDSM
        else:
...
# At the end, change the last line to remove the duplicate client_id definition:
